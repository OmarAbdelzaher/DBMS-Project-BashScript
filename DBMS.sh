...
function ConnectDatabase {
    ...
    # find if we have a database with this name in our DBMS
    findRes=`find -name $DatabaseConnect 2>>/dev/null`
...
function DropDatabase {
    ...
    if [ "./$DB" = "$findRes" ] 
        ...
        # -I : to show a prompt to the user to confirm dropping once (not with every file deletion in database)
        # -r : remove recursively the database with its contents
        rm -Ir $DB  
    else
        echo "This database doesn't exist"
    fi
...
# change directory to the DBMS folder, once the script runs
cd DBMS
